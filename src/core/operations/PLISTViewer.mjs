...
class PlistViewer extends Operation {
    ...
    constructor() {
        ...
        this.module = "Default";
        this.local="";
        this.description = "在macOS、iOS、NeXTSTEP和GNUstep编程框架中,属性列表文件是存储序列化对象的文件。属性列表文件使用文件扩展名.plist,因此通常称为p-list文件<br><br>此操作以人类可读的格式显示plist文件。In the macOS, iOS, NeXTSTEP, and GNUstep programming frameworks, property list files are files that store serialized objects. Property list files use the filename extension .plist, and thus are often referred to as p-list files.<br><br>This operation displays plist files in a human readable format.";
